...
# Сколько предметов держим в ЧАСОВОЙ слежке. 400 × 24 снимка × 31 день ≈
# 300 тысяч строк в месяц, около 19 МБ CSV. Много, но переносимо: строки
# сортируются по времени, поэтому новые дописываются в хвост и git видит
# маленький diff, а не переписанный файл.
#
# Было 80. Расширено к патчу 12.1 (2026-08-12): часовое разрешение нужно,
# чтобы ловить сливы, а на 80 позициях половина интересного рынка просто
# не видна. Широкий срез (весь аукцион 4 раза в сутки) дополняет это
# полнотой, но не даёт частоты.
SIZE = 400

# Сколько кандидатов разбирать. Справочник заполняется по два запроса на
# предмет, так что это самая долгая часть обновления списка — но она
# разовая и запускается раз в месяц.
CANDIDATES = 700
