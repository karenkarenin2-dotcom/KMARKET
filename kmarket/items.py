...
КАК РАНГ ВЫВОДИТСЯ. Единственная зацепка — поле `modified_crafting`:
у ранговых предметов оно есть и содержит `category.id`, общий для всей
группы. Внутри группы ранги идут ПОДРЯД по возрастанию ID — проверено на
травах Midnight (Кровотерн 236770/236771, Азефит 236774/236775,
Сребревица 236776/236777, Маналилия 236778/236779) и на самоцветной пыли
(242620/242621). Значит ранг — это позиция в отсортированной группе.
...
def _rank_groups(token: str, region: str, locale: str) -> None:
    ...
    # Шаг 2 — ранг это позиция в отсортированной по ID группе.
    groups: dict[int, list[int]] = {}
    for entry in data.values():
        if entry.get("category"):
            groups.setdefault(entry["category"], []).append(entry["id"])
    for category, ids in groups.items():
        ids.sort()
        for position, item_id in enumerate(ids, start=1):
            data[str(item_id)]["rank"] = position
            data[str(item_id)]["rank_of"] = len(ids)
