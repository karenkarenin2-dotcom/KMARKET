...
# Широкий срез: ВЕСЬ товарный аукцион, все ~12 тысяч предметов, несколько
# раз в сутки. Отдельно от AUCTION_DIR, потому что это другой жанр данных —
# редкий и полный против частого и узкого.
#
# ЗАЧЕМ. Список слежки подобран по сегодняшней ликвидности и новых
# реагентов патча 12.1 в нём физически нет — их ещё не существует. Если
# не записать рынок целиком через запуск сезона, потом будет нечего
# анализировать: истории у Blizzard нет, прошлое не отматывается.
# Замерено: 12 022 предмета × 4 раза в сутки ≈ 2.3 МБ в сутки.
AUCTION_WIDE_DIR = DATA_DIR / "auction_wide"

# Кэш движка WebView2. Держим его В ОДНОМ ИЗВЕСТНОМ МЕСТЕ, а не там, куда
# его кладёт pywebview по умолчанию.
#
# ПОЧЕМУ (замерено 2026-08-07). У pywebview `private_mode=True` по
# умолчанию, и на КАЖДЫЙ запуск он создаёт свежую папку в %TEMP%\tmp*,
# а при выходе её не удаляет. На машине набралось девять таких папок на
# 106 МБ, причём два сеанса KMARKET дали 30 и 69 МБ. Своя постоянная
# папка означает: кэш один, растёт он не бесконечно, и его видно.
CACHE_DIR = Path(os.environ.get("LOCALAPPDATA", str(ROOT))) / "KMARKET" / "webview"
